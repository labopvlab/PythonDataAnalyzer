...
SetallcolnamesWOids=[]
criteriaList=[]
criteriaexclusionlist=['JVmeas.commentJV', 'JVmeas.linktorawdata', 'MPPmeas.commentmpp', 
                       'MPPmeas.linktorawdata','Ncontact.commentncontacts', 'Pcontact.commentpcontacts', 
                       'PkAbsorber.commentpkabsorber','batch.commentbatch', 'electrode.commentelectrode', 
                       'environment.commentenvir', 'eqemeas.linktofile','eqemeas.commenteqe', 
                       'samples.commentsamples','recombjct.commentrecombjct', 'substtype.commentsubsttype', 
                       'users.commentusers','takencharacsetups.commenttakencharac',
                       "eqemeas.EQEmeasnameDateTimeEQEJsc","characsetups.characsetupname","characsetups.commentcharac"] #cannot restrict search from those criteria

dropdowncriteria=['JVmeas.CellArea', 'JVmeas.GroupJV', 'JVmeas.IlluminationIntensity', 'JVmeas.Operator', 
'JVmeas.ScanDirect', 'JVmeas.Setup', 'MPPmeas.CellArea', 'MPPmeas.Operator','MPPmeas.TrackingAlgo', 
'Ncontact.contactstackN', 'Pcontact.contactstackP', 'PkAbsorber.absorbercomposition', 'PkAbsorberMethod.absorberMethod',
'batch.batchname', 'batch.topic', 'cells.cellname', 'recombjct.recombjctstack', 'electrode.electrodestack','samples.bottomCellDBRef', 
'samples.cellarchitecture', 'samples.polarity', 'samples.samplename','samples.samplefullstack', 'samples.bottomCellDBRef',
'substtype.substratetype', 'users.affiliation','users.email', 'users.firstname', 'users.lastname', 
'users.username','eqemeas.EQEmeasname','takencharacsetups.takencharacsetupsname','tripletop.topstack']
fromtocriteria=['JVmeas.Delay', 'JVmeas.Eff', 'JVmeas.FF', 'JVmeas.ImaxComp', 'JVmeas.IntegTime', 
'JVmeas.Isenserange', 'JVmeas.Jmpp', 'JVmeas.Jsc', 'JVmeas.NbPoints','JVmeas.Pmpp', 'JVmeas.Roc', 
'JVmeas.Rsc', 'JVmeas.Vend', 'JVmeas.Vmpp', 'JVmeas.Voc', 'JVmeas.Vstart', 'MPPmeas.PowerAvg', 
'MPPmeas.PowerEnd','MPPmeas.TrackingDuration', 'MPPmeas.Vstart', 'MPPmeas.Vstep', 
'environment.RHMC162', 'environment.RHyellowroom', 'environment.Tempmc162', 
'environment.Tempyellowroom','environment.evapGBoxygenvalue', 'environment.evapGBwatervalue', 
'environment.gloveboxsolvent', 'environment.solventGBoxygenvalue', 'environment.solventGBwatervalue', 
'eqemeas.Eg', 'eqemeas.EgLn', 'eqemeas.EgTauc', 'eqemeas.integJsc']
timecriteria=['JVmeas.DateTimeJV', 'MPPmeas.DateTimeMPP', 'batch.startdate','eqemeas.DateTimeEQE',]
...
#               command = self.test, width=100).pack()
    ...
    def on_start(self):
        ...
        try:
            path =filedialog.askopenfilenames(title="Please select the Database .db file")[0]
...
#        self.listboxsamples.config(yscrollcommand=scrollbar.set)
    ...
    def addtheusualparameters(self):
        ...
        usualparam=['samples.samplename','users.username','JVmeas.CellArea','JVmeas.DateTimeJV',
...
################################################################################             
    ...
    def SearchingAndExporting(self):
        ...
        self.theCursor.execute("""SELECT batchname, startdate, users.username
                          FROM batch
    ...
    #            print(wherelist)
                listwosamplename=["batch", "environment", "users", "takencharacsetups"]
